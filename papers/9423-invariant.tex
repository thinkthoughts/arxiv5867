...
\begin{abstract}
We identify a previously unnoticed invariant in sieve-theoretic counting on primorial-constrained residue classes. For numbers congruent to 5 modulo 6 and coprime to a primorial $P_k\#$, the ratio of the actual count to the predicted density from the totient function converges to $24/25$ as the upper limit increases, leaving a persistent $1/25$ gap. This deviation is not asymptotic noise but a fixed structural residue, arising from a cyclic constraint we denote as the \textbf{9423 phase‑lock}. The invariant holds across all tested primorial scales and resists collapse under parity-based obstructions. We present numerical verification and conjecture an algebraic proof via the 9423 cycle's action on admissible tuples.
\end{abstract}
...
\section{The 9423 Cycle}

The invariant takes its name from the cyclic constraint:

\[
9423\pi \quad \text{with cycle} \quad [9,4,2,3],
\]
...
\begin{itemize}
    ...
    \item The $9423$ cyclic constraint,
\end{itemize}
...
We have exhibited a persistent $1/25$ gap in primorial‑constrained counting, linked to a cyclic phase‑lock invariant. The result is numerically stable across scales and offers a precise quantification of what sieve methods systematically undercount. An algebraic proof remains open.
